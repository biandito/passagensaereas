	    ...
	    //MENU RESERVA DE PASSAGENS
	     case 1:
	    	 ...
	    	 case 0:
	    		 ...
	    		 for(int j = 0; j<nVoo; j++) {if((voos[j].aeronave.modelo+", "+ voos[j].data+", " + voos[j].hora).equals(escVooStr)){
	    			 escVoo= voos[j];}}
	    		 
	    		 String nome, cpf;
	    		 nome = JOptionPane.showInputDialog("Digite seu nome");
	    		 cpf = JOptionPane.showInputDialog("Digite seu cpf");
	    		 Passageiro passa = new Passageiro(nome, cpf);
	    		 int lugar[] = new int[2];
	    		 ...
	    		 escVoo.aeronave.setPassageiro(lugar[0]-1, lugar[1]-1, passa);
	    		 break;
	    		 
	    	 //CONSULTAR NUMERO DE LUGARES VAZIOS
	    	 case 1:
	    		 String verVooStr[] = new String[nVoo];
	    		 for(int i = 0; i<nVoo; i++) {verVooStr[i]=voos[i].aeronave.modelo+", "+ voos[i].data+", " + voos[i].hora;}
	    		 String verStr = (String) JOptionPane.showInputDialog(null, "Escolha um voo", "Escolher Voo", nVoo, null, verVooStr, verVooStr[0]);
	    		 Voo verVoo = new Voo();
	    		 for(int j = 0; j<nVoo; j++) {if((voos[j].aeronave.modelo+", "+ voos[j].data+", " + voos[j].hora).equals(verStr)){
	    			 verVoo= voos[j];}}
	    		 /*Voo verVoo = (Voo) JOptionPane.showInputDialog(null, "Escolha um voo", "Escolher Voo", nVoo, null, voos, voos[0]);*/
	    		 int nLugares = 0;
	    		 for(int i = 0; i< verVoo.aeronave.Lugares.length; i++) {
	    			 for(int j = 0; j<verVoo.aeronave.Lugares[0].length; j++) {
	    				 
	    				 if(!verVoo.aeronave.verificaLugarOcupado(i, j)) {nLugares++;}
